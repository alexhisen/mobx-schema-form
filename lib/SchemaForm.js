...
var SchemaForm = (0, _mobxReact.observer)(_class = function (_React$Component) {
  ...
  function SchemaForm() {
    ...
    return _ret = (_temp = (_this = _possibleConstructorReturn(this, (_ref = SchemaForm.__proto__ || Object.getPrototypeOf(SchemaForm)).call.apply(_ref, [this].concat(args))), _this), _this.onModelChange = function (formField, value) {
      ...
      var asString = true;
      var modelKey = (0, _validate.getFieldKey)(formField, asString);
      if (!modelKey) {
        _this.props.onModelChange && _this.props.onModelChange(formField, value);
        return;
      }

      _this.props.onModelChange && _this.props.onModelChange(modelKey, value);

      var ignoreModelKey = true;
      var fieldKey = (0, _validate.getFieldKey)(formField, asString, ignoreModelKey);

      // when whole form unmounts (or schema changes and no longer includes us),
      // just remove us from model.fields and bail out.
      if (_this.unmounting) {
        if (model.fields && fieldKey === (0, _validate.getFieldKey)(model.fields[modelKey], asString, ignoreModelKey)) {
      ...
      if (value === undefined) {
        ...
        if (form.falseConditionValue !== undefined && form.condition && eval(form.condition) === false) {
          (0, _mobx.action)(function () {
            var key = (0, _validate.getFieldKey)(form);
            _utils2.default.selectOrSet(key, model.data, form.falseConditionValue);
            _utils2.default.selectOrSet(key, model.dataErrors, null);
          })();
        }
        /* eslint-enable */
        if (fieldKey === (0, _validate.getFieldKey)(model.fields[modelKey], asString, ignoreModelKey)) {
          delete model.fields[modelKey];
        }
      } else {
  ...
  _createClass(SchemaForm, [{
    ...
    value: function builder(form, model, index, onChange, mapper) {
      var asString = true;
      var ignoreModelKey = true;
      var key = (0, _validate.getFieldKey)(form, asString, ignoreModelKey);
