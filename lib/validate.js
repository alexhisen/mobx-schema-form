...
function getFieldValue(formField, model) {
  ...
  if (value === null || value === undefined) {
    ...
    if (!formField.titleMap && formField.schema && formField.schema.enum) {
      ...
      formField.titleMap = _reactSchemaForm.utils.enumToTitleMap(formField.schema.enum);
    }
...
function validateField(formField, model, value) {
  ...
  (0, _mobx.action)(function () {
    ...
    // update error
    _reactSchemaForm.utils.selectOrSet(getFieldKey(formField)[0], // for nested props. e.g. if type is array of autocmpletes
